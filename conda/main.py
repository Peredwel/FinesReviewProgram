...
HEADERS = [
    'lender_institution', 'borrower_institution', 'full_name', 'user_email',
    'expiry_date', 'remaining_amount', 'active_loan_count'
]
...
def detect_delimiter(file_path: str, encoding: str) -> str:
    sniffer = csv.Sniffer()
    data = open(file_path, "r", encoding=encoding).read(1024)
...
def checkcsv(filename):
    ...
    delimiter = detect_delimiter(myfile, encoding)
    df = pd.read_csv(myfile, delimiter=delimiter, header=0, encoding=encoding)
...
def convertcsvtoxlsx(filename):
    ...
    read_file.to_csv('InputAFNFiles_csv/' + newfilename,
                     index=None,
                     header=True)
    #delete xlsx file
...
def createReportFiles():
    ...
    with open('AFNSchools.csv', encoding='UTF-8') as csv_file:
        ...
        for row in csv_reader:
            school = row[0]
            location = 'OutputFiles/' + school.strip().replace(
                '/', '').replace(' ', '_') + '.csv'
...
def populateReport(filedict):
    for file in os.listdir('InputAFNFiles_csv'):
        ...
        with open(filepath, 'r', encoding=encoding) as csv_file1:
            csv_reader = csv.reader(csv_file1, delimiter=delimiter)
            for row in csv_reader:
                if line_count > 0:
                    lender = row[0]
                    borrower = row[1]
                    ...
                    if borrower != "":
                        ...
                        with open("OutputFiles/errors.csv",
                                  'a',
                                  encoding='UTF-8',
                                  newline='\n') as writefile:
                            csv_writer = csv.writer(writefile, delimiter='\t')
